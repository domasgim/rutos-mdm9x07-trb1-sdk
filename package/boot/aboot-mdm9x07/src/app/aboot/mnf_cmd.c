...
static mnf_cmd_flag_map_t mnf_cmd_flag_map[] = { { MNFCMD_CONFIRM, "CONFIRM" },
						 { MNFCMD_FORCE, "FORCE" },
						 { 0 } };
...
static int print_usage_help()
{
	...
	MNFINFO_PRINT("    CONFIRM                       - Apply any changes");
...
static int mnf_cmd_write_finalize(mnf_cmd_flags flags)
{
	if (!(flags & MNFCMD_CONFIRM)) {
		...
		MNFINFO_PRINT("");
		MNFINFO_RESULT_ERR("Rerun the command with 'CONFIRM' to apply the changes");
		return 1;
	}
...
void parse_cmdline(char *cmdline, int *argc, char **argv, int argc_max)
{
	char *arg;
	*argc = 0;

	if (*cmdline == ' ')
		cmdline++; // Skip the first character if it's a space

	if (*cmdline == '\0')
		return; // No args, skip

	for (arg = cmdline; *cmdline != '\0' && *argc < argc_max; cmdline++) {
		if (*cmdline == ' ') {
			argv[*argc] = arg;
			(*argc)++;
			*cmdline = '\0';
...
void cmd_mnfinfo(const char *arg, void *data, unsigned sz)
{
	const char *argv[16];
	int argc;

	parse_cmdline((char *)arg, &argc, argv, ARRAY_SIZE(argv));

	mnf_cmd_entry(argc, argv);
}