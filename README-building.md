...
The helper script is available at: `TRB1_R_GPL_00.07.23.3/scripts/dockerbuild`.
...
2. Extract the  archive to an empty folder
   ...
   ```sh
   mkdir TRB1_R_GPL_00.07.23.3
   tar -xzf ~/Downloads/TRB1_R_GPL_00.07.23.3.tar.gz -C TRB1_R_GPL_00.07.23.3
   cd TRB1_R_GPL_00.07.23.3
   ```
...
5. Prepare packages for offline installation (optional)
   ...
   After this you can find packages in "TRB1_R_GPL_00.07.23.3/bin/packages/<arch_name>/zipped_packages".
...
After successful build you will get the firmware file in `TRB1_R_GPL_00.07.23.3/bin/targets/mdm9x07/generic/tltFws`.
...
```text
TRB1_R_GPL_00.07.23.3/package/feeds/vuci/vuci-ui-core/bin/dist
```
...
2. Generate banner text:
   ...
   ```sh
   figlet YOUR_TEXT > "TRB1_R_GPL_00.07.23.3/package/base-files/files/etc/banner.logo"
   ```
...
1. Open the device family file:
   ...
   ```text
   TRB1_R_GPL_00.07.23.3/target/linux/mdm9x07/image/devices/trb1xx_family.mk
   ```
...
4. Regenerate tmpinfo files:
   ...
   ```sh
   rm -rf "TRB1_R_GPL_00.07.23.3/tmp"
   ```
...
```text
TRB1_R_GPL_00.07.23.3/package/base-files/files/lib/preinit/84_set_password
```
