...
void *smem_find(unsigned id, unsigned size_in, unsigned to_proc, unsigned flags)
{
	 ...
	 */
	ptr = qcom_smem_get(to_proc, id, &size);
	if (!ptr)
		return 0;

	size_in = ALIGN(size_in, 8);
	if (size_in != size) {
		SMSM_INFO("smem_find(%u, %u, %u, %u): wrong size %u\n",
...
static void smd_channel_probe_now(struct remote_proc_info *r_info)
{
	...
	shared = qcom_smem_get(r_info->remote_pid, ID_CH_ALLOC_TBL, &tbl_size);
	...
	shared = qcom_smem_get(r_info->remote_pid, SMEM_CHANNEL_ALLOC_TBL_2, &tbl_size);
...
void smd_channel_reset(uint32_t restart_pid)
{
	...
	shared_pri = qcom_smem_get(restart_pid, ID_CH_ALLOC_TBL, &pri_size);
	if (IS_ERR(shared_pri)) {
		pr_err("%s: allocation table not initialized\n", __func__);
		return;
	}
	shared_sec = qcom_smem_get(restart_pid, SMEM_CHANNEL_ALLOC_TBL_2, &sec_size);
...
static void do_smd_probe(unsigned remote_pid)
{
	...
	free_space = qcom_smem_get_free_space(remote_pid);
						...
						struct remote_proc_info *r_info)
{
	...
	buffer = qcom_smem_get(r_info->remote_pid, fifo_id + ch->n, &buffer_sz);
	if (IS_ERR(buffer)) {
...
static int smsm_init(void)
{
	...
	if (!smsm_info.state) {
		qcom_smem_alloc(0, ID_SHARED_STATE, SMSM_NUM_ENTRIES * sizeof(uint32_t));
		unsigned sz;
		smsm_info.state = qcom_smem_get(0, ID_SHARED_STATE, &sz);

		if (!IS_ERR(smsm_info.state))
			__raw_writel(0, SMSM_STATE_ADDR(SMSM_APPS_STATE));
	}

	if (!smsm_info.intr_mask) {
		qcom_smem_alloc(0, SMEM_SMSM_CPU_INTR_MASK, SMSM_NUM_ENTRIES * SMSM_NUM_HOSTS * sizeof(uint32_t));
		unsigned sz;
		smsm_info.intr_mask = qcom_smem_get(0, SMEM_SMSM_CPU_INTR_MASK, &sz);

		if (!IS_ERR(smsm_info.intr_mask)) {
