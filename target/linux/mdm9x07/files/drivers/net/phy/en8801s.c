...
struct en8801s_priv {
	...
	int led_state;
	struct dentry *deb_dir;
	struct dentry *deb_file;
#if (KERNEL_VERSION(4, 16, 0) > LINUX_VERSION_CODE)
...
LED6 10/100M/LINK/ACT (GPIO0)  <-> BASE_T_LED1,
LED4 100M/LINK/ACT    (GPIO1)  <-> BASE_T_LED2,
*/
/* User-defined.B */
...
*/
	/* UNUSED */
	{LED_DISABLE, 5, AIR_ACTIVE_LOW,
	 BASE_T_LED0_ON_CFG, BASE_T_LED0_BLK_CFG},
	/* UNUSED */
	{LED_DISABLE, 9, AIR_ACTIVE_LOW,
	 BASE_T_LED0_ON_CFG, BASE_T_LED0_BLK_CFG},
	/* YELLOW */
	{LED_ENABLE, 0, AIR_ACTIVE_HIGH,
	 BASE_T_LED1_ON_CFG, BASE_T_LED1_BLK_CFG},
	/*  GREEN */
	{LED_ENABLE, 1, AIR_ACTIVE_LOW,
	 BASE_T_LED0_ON_CFG, BASE_T_LED0_BLK_CFG},
};
...
static int airoha_efuse_write(struct mii_bus *mbus, int pbus_addr,
			int offset, unsigned int *efuse_data)
{
	int ret = 0;
	unsigned int data = 0;

	ret = airoha_pbus_write(mbus, pbus_addr, 0x1c10, *(efuse_data));
	if (ret < 0)
		return ret;
	ret = airoha_pbus_write(mbus, pbus_addr, 0x1c14, *(efuse_data + 1));
	if (ret < 0)
		return ret;
	ret = airoha_pbus_write(mbus, pbus_addr, 0x1c18, *(efuse_data + 2));
	if (ret < 0)
		return ret;
	ret = airoha_pbus_write(mbus, pbus_addr, 0x1c1c, *(efuse_data + 3));
	if (ret < 0)
		return ret;
	ret = airoha_pbus_write(mbus, pbus_addr, 0x1c0c, *(efuse_data + 4));
	if (ret < 0)
		return ret;
	data = (offset << 16) | 0x40000000 | (0x3 << 6);
	ret = airoha_pbus_write(mbus, pbus_addr, 0x1c08, data);
	if (ret < 0)
		return ret;
...
static int airoha_efuse_read(struct mii_bus *mbus, int pbus_addr,
			int offset, unsigned int *efuse_data)
{
	int data = 0, ret = 0;

	data = (offset << 16) | 0x40000000 | (0x1 << 6);
	ret = airoha_pbus_write(mbus, pbus_addr, 0x1c08, data);
	if (ret < 0)
		return ret;
	*(efuse_data)     = airoha_pbus_read(mbus, pbus_addr, 0x1c30);
	*(efuse_data + 1) = airoha_pbus_read(mbus, pbus_addr, 0x1c34);
	*(efuse_data + 2) = airoha_pbus_read(mbus, pbus_addr, 0x1c38);
	*(efuse_data + 3) = airoha_pbus_read(mbus, pbus_addr, 0x1c3c);
	*(efuse_data + 4) = airoha_pbus_read(mbus, pbus_addr, 0x1c2c);
			...
			int polar, u16 on_evt, u16 blk_evt)
{
	...
	if (priv->led_state != EN8801S_PORT_LED_STATUS)
	{
		on_evt = 0;
		blk_evt = 0;
	}
	...
	if (priv->led_state == EN8801S_PORT_LED_ON)
		on_evt ^= LED_ON_POL;
...
static void en8801s_led_set_mode(struct led_classdev *cdev, enum led_brightness mode)
{
	struct en8801s_priv *priv = container_of(cdev, struct en8801s_priv, led);
	if (mode < 0)
		mode = 0;
	else if ( mode > 0)
		mode = 2;
	priv->led_state = mode;
	en8801s_led_init(priv->phydev);
}

static enum led_brightness en8801s_led_get_brightness(struct led_classdev *cdev)
{
	struct en8801s_priv *priv = container_of(cdev, struct en8801s_priv, led);
	return priv->led_state;
}

static void en8801s_debugfs_create(struct phy_device *phydev)
{
	struct device *dev = phydev_dev(phydev);
	struct en8801s_priv *priv = phydev->priv;
	
	priv->deb_dir = debugfs_create_dir("en8801_phy", 0);
	if (!priv->deb_dir) {
		dev_err(dev, "Unable to create debugfs dir\n");
		return;
	}
	priv->deb_file = debugfs_create_u16("version", 0444, priv->deb_dir,
					&priv->version);
	if (!priv->deb_dir) {
		dev_err(dev, "Unable to create debugfs file\n");
		return;
	}
}
...
static int en8801s_probe(struct phy_device *phydev)
{
	...
	priv->phydev = phydev;
	priv->led_state = EN8801S_PORT_LED_STATUS;
	priv->led.name = "eth_led";
	priv->led.max_brightness = 2;
	priv->led.brightness_set = en8801s_led_set_mode;
	priv->led.brightness_get = en8801s_led_get_brightness;
		
	ret = led_classdev_register(dev, &priv->led);
	if (ret < 0)
		dev_err(dev, "en8801s: LED registration failed (%d)\n", ret);
	en8801s_debugfs_create(phydev);	
	return 0;
}
