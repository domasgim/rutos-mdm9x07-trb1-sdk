...
static int msm_nand_parse_smem_ptable(int *nr_parts)
{
	...
	*nr_parts = ptable.numparts;
	...
	for (i = 0; i < ptable.numparts; i++) {
		...
		// trb1: make MNF partition read-only
		if (!strcmp(mtd_part[i].name, "mnf_info"))
			mtd_part[i].mask_flags |= MTD_WRITEABLE;
...
static int msm_nand_probe(struct platform_device *pdev)
{
	...
	/* quectel, use old interface of nand */
	info->nand_chip.caps = 0x00;
