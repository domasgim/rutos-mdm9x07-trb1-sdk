...
static irqreturn_t gpio_usbdetect_vbus_irq(int irq, void *data)
{
	...
	ret = irq_get_irqchip_state(irq, IRQCHIP_STATE_LINE_LEVEL, &vbus);
	...
	if (vbus) {
		...
		power_supply_set_property(usb->usb_psy, POWER_SUPPLY_PROP_TYPE, &prop);
	} else {
	...
	prop.intval = vbus;
	power_supply_set_property(usb->usb_psy, POWER_SUPPLY_PROP_PRESENT, &prop);
