...
bool quec_udc_is_resume(void)
{
	...
	if (udc == NULL || !udc->vbus_active) {
		return false;
	}
	
	if (udc->suspended == 0 && strcmp("ci13xxx_msm", udc->udc_driver->name) == 0) {
		return true;
	}
...
void quec_force_udc_suspend(void) 
{
	...
	if (!quec_udc_is_resume())
		return;
	
	udc->driver->suspend(&udc->gadget);
	if (udc->udc_driver->notify_event) {
		udc->udc_driver->notify_event(udc, CI13XXX_CONTROLLER_SUSPEND_EVENT);
	}
	if (udc->transceiver) {
