...
#define FORCE_USB_SUSPEND_DELAY    (30000)

bool quectel_otg_resume = true;
static struct mutex otg_resume_mutex;
static bool force_usb_suspend = false;
void quec_force_udc_suspend(void);
bool quec_udc_is_resume(void);
#endif
...
#ifdef QUECTEL_SLEEP_CTRL
static bool usb_port_suspend(struct msm_otg *motg)
{
	/*
	 * USB_PORTSC BIT7(SUSP)
	...
	return !!(readl_relaxed(USB_PORTSC) & BIT(7));
}

static void force_usb_suspend_work_cb(struct work_struct *w)
{
	struct msm_otg *motg = container_of(w, struct msm_otg, 
		force_usb_suspend_work.work);
	
	bool suspend = false;

	force_usb_suspend = false;
	if (motg && motg->phy.otg->gadget->speed == USB_SPEED_UNKNOWN
		&& quec_udc_is_resume()
		&& !motg->vbus_state) {
		...
		suspend = usb_port_suspend(motg);
		mutex_unlock(&otg_resume_mutex);

		if (suspend) {
			force_usb_suspend = true;
			quec_force_udc_suspend();
		} else {
			queue_delayed_work(motg->otg_wq, &motg->force_usb_suspend_work,
				msecs_to_jiffies(FORCE_USB_SUSPEND_DELAY));
		}
	}
}
#endif
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	if(motg->pdata->mode == USB_PERIPHERAL) {
		if (force_usb_suspend && !usb_port_suspend(motg)) {
			ret = -EBUSY;
			force_usb_suspend = false;
			queue_delayed_work(motg->otg_wq, &motg->force_usb_suspend_work,
				msecs_to_jiffies(FORCE_USB_SUSPEND_DELAY));
...
#endif
#ifdef QUECTEL_SLEEP_CTRL
    if (phy->otg->state == OTG_STATE_B_SUSPEND && !usb_port_suspend(motg))
    {
	ret = -EBUSY;
        goto phy_suspend_fail;
    }
...
static int msm_otg_resume(struct msm_otg *motg)
{
	...
	if (phy->otg->state != OTG_STATE_B_SUSPEND)
	{
...
{
	union power_supply_propval propval;
	union power_supply_propval propval2;
	...
	if (motg->cur_power == 0 && mA > 2) {
		/* Enable charging */
		propval.intval = true;
		propval2.intval = 1000*mA;
		if (power_supply_set_property(psy, POWER_SUPPLY_PROP_ONLINE, &propval))
			goto psy_error;
		if (power_supply_set_property(psy, POWER_SUPPLY_PROP_CURRENT_MAX, &propval2))
			goto psy_error;
...
#if 0 //add by carl.yin 20180703, this maybe detect some usb hubs as charger, and make usb diconnect
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	if (motg->resume_pending) {
		pm_runtime_get_noresume(otg->usb_phy->dev);
		msm_otg_resume(motg);
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPE:
		val->intval = motg->usb_psy_desc.type;
		break;
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_TYPE:
		motg->usb_psy_desc.type = val->intval;
					...
					struct power_supply_config *psy_cfg)
{
	...
	motg->usb_psy = power_supply_register(&pdev->dev, &motg->usb_psy_desc, psy_cfg);
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	timer_setup(&motg->chg_check_timer, msm_otg_chg_check_timer_func, 0);
#ifdef QUECTEL_SLEEP_CTRL
	INIT_DELAYED_WORK(&motg->force_usb_suspend_work, force_usb_suspend_work_cb);
#endif

#if 0
	motg->otg_wq = alloc_ordered_workqueue("k_otg", 0);
#else
	motg->otg_wq = alloc_ordered_workqueue("k_otg", WQ_HIGHPRI);
#endif
	if (!motg->otg_wq) {
	...
	psy_cfg.drv_data = motg;
	psy_cfg.supplied_to = otg_pm_power_supplied_to;
	psy_cfg.num_supplicants = ARRAY_SIZE(otg_pm_power_supplied_to);
	...
	if (!msm_otg_register_power_supply(pdev, motg, &psy_cfg))
		psy = motg->usb_psy;

	power_supply_powers(motg->usb_psy, &pdev->dev);
...
#ifdef QUECTEL_SLEEP_CTRL
	cancel_delayed_work_sync(&motg->force_usb_suspend_work);
#endif
	destroy_workqueue(motg->otg_wq);
