...
#ifdef QUECTEL_UART_SUSPEND
	wakeup_source_remove(msm_uport->serial_ws);
	wakeup_source_destroy(msm_uport->serial_ws);
#endif
...
void tx_timeout_handler(struct timer_list *t)
{
	...
	struct msm_hs_port *msm_uport = tx->parent;
	struct uart_port *uport = &msm_uport->uport;
	int isr;
...
static int msm_hs_probe(struct platform_device *pdev)
{
	...
	if (!ret) {
#ifdef QUECTEL_UART_SUSPEND
		msm_uport->serial_ws = wakeup_source_create(dev_name(&pdev->dev));
		wakeup_source_add(msm_uport->serial_ws);
#endif
		msm_hs_clk_bus_unvote(msm_uport);
