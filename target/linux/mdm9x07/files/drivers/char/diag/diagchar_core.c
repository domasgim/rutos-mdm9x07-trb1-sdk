...
void diag_get_timestamp(char *time_str)
{
	...
	if (!time_str)
		return;
	time64_to_tm(ts.tv_sec, 0, &broken_tm);
	scnprintf(time_str, DIAG_TS_SIZE, "%d:%d:%d:%ld", broken_tm.tm_hour,
				broken_tm.tm_min, broken_tm.tm_sec, ts.tv_nsec / 1000);
}
...
static int diagchar_setup_cdev(dev_t devno)
{
	...
	driver->diag_dev->power.wakeup = wakeup_source_create("DIAG_WS");
	wakeup_source_add(driver->diag_dev->power.wakeup);
	return 0;
...
static int __init diagchar_init(void)
{
	...
	timer_setup(&drain_timer, drain_timer_func, 1234);
